...
function calculateCountOfCards(lines: string[]): number {
  const cards = new Array(lines.length).fill(1)

  for (let i = 0; i < lines.length; i++) {
    const matches = countNumberOfMatchesOnCard(lines[i])
...
describe('calculateCountOfCards (part 2)', () => {
  it('should handle the example input', () => {
    const input = [
      'Card 1: 41 48 83 86 17 | 83 86  6 31 17  9 48 53',
      'Card 2: 13 32 20 16 61 | 61 30 68 82 17 32 24 19',
    ...
    expect(calculateCountOfCards(input)).toBe(30)
  })

  it('should get the answer for part 2', async () => {
    const input = await readInputIntoLines('day-04')
    const answer = calculateCountOfCards(input)
    expect(answer).toBe(9496801)
  })
})
